...
public final class ODEsolver
{
	...
	public static Network timeStep(Network network, Disease disease, double h)
	{
		...
		{
			kS[0][i] = h
			      * (-tempS[i] * (beta * tempI[i] + gamma * tempN[i]) / tempN[i]
			            + travelSum(commute, tempS, i));
			kI[0][i] = h
			      * (tempI[i] * (beta * tempS[i] - alpha * tempN[i]) / tempN[i]
		...
		for (int i = 0; i < nlen; i++)
		{
			nodes[i].setS(svals[i] + kS[0][i] / 6 + kS[1][i] / 3 + kS[2][i] / 3
			      + kS[3][i] / 6);
			nodes[i].setI(ivals[i] + kI[0][i] / 6 + kI[1][i] / 3 + kI[2][i] / 3
			      + kI[3][i] / 6);
			nodes[i].setR(rvals[i] + kR[0][i] / 6 + kR[1][i] / 3 + kR[2][i] / 3
			      + kR[3][i] / 6);
		}
