public class ODEsolver {
	...
	/*
	 * Calcuated the # of people in {S, I ,R} that travels to the node w/ index
	 * n
	 */
	private static double travelSum(double[][] commute, double[] numppl, int n) {
		double sum = 0;
		for (int m = 0; m < numppl.length; m++) {
			sum += commute[m][n] * numppl[m];
			sum -= commute[n][m] * numppl[n];
		}
	...
	public static Network timeStep(Network network, Disease disease, double h) {
		...
		/*
		 * for (int i = 0; i < nlen; i++) for (int j = 0; j < nlen; j++){
		 * System.out.println(commute[i][j]*nodes[i].getN() - commute[j][i] *
		 * nodes[j].getN()); System.out.println(); }
		 */
