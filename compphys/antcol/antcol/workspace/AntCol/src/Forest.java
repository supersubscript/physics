...
public class Forest
{
   ...
   public static void main(String[] args)
   {
      ...
      while (iterations-- > 0)
      {
         ...
         System.out.print(Common.ITERATIONS - iterations + "\t" + spHill + "\t"
               + brHill + "\t");
         forr.timeStep();
      }
   }
   ...
   public void getFood()
   {
      Iterator<Food> i = food.iterator(); // Avoid stupid concurrency errors
      while (i.hasNext())
      {
         Food f = i.next();
   ...
   {
      // Movement mega matrix. Needed not to take multiple steps for some ants
      // when traversing matrix.
      int[][][][][] transfer = new int[squares.length][squares.length][4][2][2];
      ...
      for (int i = 0; i < squares.length; i++)
         for (int j = 0; j < squares[0].length; j++)
         {
            ...
            if (j != squares.length - 1)
               sq.transferAnts(squares[i][j + 1], transfer[i][j][0]);
            if (j != 0)
               sq.transferAnts(squares[i][j - 1], transfer[i][j][1]);
            if (i != 0)
               sq.transferAnts(squares[i - 1][j], transfer[i][j][2]);
            if (i != squares[0].length - 1)
               sq.transferAnts(squares[i + 1][j], transfer[i][j][3]);
         }
   }
   ...
   public void timeStep()
   {
      ...
      try
      {
         Thread.sleep(0); // Add sleep time for graphics if you want
