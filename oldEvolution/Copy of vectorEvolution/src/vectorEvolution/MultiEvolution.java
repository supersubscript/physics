...
public class MultiEvolution
{
	...
	static HashMap<String, Double> bitstringValues = new HashMap<String, Double>();
	...
	public static void init(String[] args) throws IOException
	{
		...
		for (int i = 0; i < SIMULATIONS; i++)
		{
			...
			for (int j = 0; j < POPULATION_SIZE; j++)
				p.put(new Bitstring(GENOME_LENGTH), Double.MAX_VALUE);
		}

		for (int i = 0; i < 1024; i++)
		{
			String s = Integer.toBinaryString(i);
			bitstringValues.put(s, encoding.value(s));
		}
		for(Entry<String, Double> d : bitstringValues.entrySet())
