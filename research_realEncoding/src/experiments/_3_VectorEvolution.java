...
public class _3_VectorEvolution {
	...
	public static void main(String[] args) {
		...
		Sequence separator = new Sequence(new boolean[]{false, true, true, true, true, true, false});
		int    valueDiam   = 3;
...
//		RealReader reader  = RealReader.multiplicative(Distribution.uniform(-2., 2.));
		...
		FitnessFunction<organisms.Vector> fitness = (n) -> {
			...
			for (double v : n.getValues()) {
				if (i>1) {
					...
					val = val > 2.
							? 0.
							: val * Math.sin(10. * Math.PI * val) + 0.5;
					result += val;
					i++;
				}
			}
		...
		MutationOperator mut = MutationOperator.combine(
				...
				MutationOperator.poisson_indel(indelRate, indelRate, indelRate, indelRate, indelRate, indelRate, indelRate, indelRate, indelRate, indelRate)
				);
		Distribution<Integer> initialSizeDistribution = Distribution.constant(initialSize);
		
		settings.setOrganismFactory(new organisms.Vector.Factory(
				mut,
				initialSizeDistribution,
				reader,
				crossover,
				separator));
		settings.setSplittingRule(SelectionRule.selectProportion(fitness, 1-sensuality));
		settings.setSexyRule(PairSelectionRule.any(SelectionRule.selectProportion(fitness, 2*sensuality)));
...
//			for (organisms.Number o : w.population()) directFit.add(fitness.applyDirectly(o));
